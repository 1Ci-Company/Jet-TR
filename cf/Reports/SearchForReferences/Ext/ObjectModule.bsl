...
Function TitleByRefSet(Val RefSet)
	...
	If TypeOf(RefSet) = Type("ValueList") Then
		...
		If TotalRefs = 1 Then
			Return StringFunctionsClientServer.SubstituteParametersToString(
				NStr("en = 'Occurrences of %1';tr = 'Kullanım yerleri %1'"), Common.SubjectString(RefSet[0].Value));
		ElsIf TotalRefs > 1 Then
			...
			If EqualType Then
				Return StringFunctionsClientServer.SubstituteParametersToString(
					NStr("en = 'Occurrences of %1 (%2)';tr = 'Nesne kullanım konumları ""%1"" (%2)'"), 
					RefSet[0].Value.Metadata().Presentation(),
					TotalRefs);
			Else		
				Return StringFunctionsClientServer.SubstituteParametersToString(
					NStr("en = 'Occurrences (%1)';tr = 'Nesne kullanım konumları (%1)'"), 
					TotalRefs);
			EndIf;
		EndIf;
	...
	Return NStr("en = 'Item occurrences';tr = 'Öğe kullanım yerleri'");
