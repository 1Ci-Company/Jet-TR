...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.IsMobileClient() Then
		...
		Items.FormCreateFromScanner.Title = NStr("en = 'From device camera…';tr = 'Cihazın kamerasından...'");
	EndIf;
...
Procedure MoveToFolder(Command)
	...
	FormParameters = New Structure;
	FormParameters.Insert("Title",    NStr("en = 'Select folder';tr = 'Klasör seçimi'"));
	...
	ElsIf TypeOf(DragValue) = Type("Array") Then
		...
		ElsIf ValueType = Type("CatalogRef.Files") Then
			...
			If Action = DragAction.Copy Then
				...
				If DragValue.Count() = 1 Then
					NotificationTitle1 = NStr("en = 'File copied.';tr = 'Dosya kopyalandı.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("en = 'File ""%1"" 
						           |is copied to folder ""%2"".';tr = '""%1""
						           |dosya ""%2"" klasöre kopyalandı'"),
						DragValue[0],
						String(FolderForAdding));
				Else
					NotificationTitle1 = NStr("en = 'Files copied.';tr = 'Dosyalar kopyalandı.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("en = '%1 files are copied to folder ""%2.""';tr = 'Dosyalar (%1 adet) ""%2"" klasörüne kopyalandı'"),
						DragValue.Count(),
						String(FolderForAdding));
				EndIf;
			...
			Else
				...
				If DragValue.Count() = 1 Then
					NotificationTitle1 = NStr("en = 'File moved.';tr = 'Dosya taşındı'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("en = 'File ""%1"" 
						           |is moved to folder ""%2.""';tr = '""%1""
						           |dosya ""%2"" klasöre taşındı'"),
						String(DragValue[0]),
						String(FolderForAdding));
				Else
					NotificationTitle1 = NStr("en = 'Files moved.';tr = 'Dosyalar taşındı.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("en = '%1 files are moved to folder ""%2.""';tr = 'Dosyalar (%1 adet) ""%2"" klasörüne taşındı'"),
		...
		ElsIf ValueType = Type("CatalogRef.FilesFolders") Then
				...
				If LoopFound = True Then
					If DragValue.Count() = 1 Then
						MessageText = NStr("en = 'Cannot move the folder.
							|The ""%1"" folder is subordinate to the ""%2"" folder that you want to move.';tr = 'Taşınamaz.
							|Klasör ""%1"" taşınan ""%2"" klasörün alt klasörüdür.'");
					Else
						MessageText = NStr("en = 'Cannot move the folder.
							|The ""%1"" folder is subordinate to one of the folders that you want to move.';tr = 'Taşınamaz.
			...
			If DragValue.Count() = 1 Then
				...
				NotificationTitle1 = NStr("en = 'Folder moved.';tr = 'Klasör taşındı.'");
				NotificationText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("en = 'Folder ""%1""
					           |is moved to folder ""%2.""';tr = '""%1""
					...
					String(FolderForAdding));
			Else
				NotificationTitle1 = NStr("en = 'Folders moved.';tr = 'Klasörler taşındı.'");
				NotificationText = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("en = '%1 folders are moved to folder ""%2.""';tr = 'Klasörler (%1 adet) ""%2"" klasörüne taşındı'"),
					String(DragValue.Count()),
					String(FolderForAdding));
			EndIf;
			ShowUserNotification(NotificationTitle1, , NotificationText, PictureLib.DialogInformation);
		...
		If Items.Folders.CurrentData.FolderSynchronizationEnabled Then
			...
			If ValueIsFilled(SynchronizationInfo) Then
				...
				Items.DecorationNote.Title = StringFunctionsClient.FormattedString(
					NStr("en = 'The files are stored in cloud service <a href=""%1"">%2</a>.';tr = 'Dosyalarla çalışma bulut hizmetinde gerçekleştirilir  <a href=""%1"">%2</a>.'"),
			...
			Else
				...
				Items.DecorationNote.Title = 
					NStr("en = 'The files are stored in cloud service.';tr = 'Dosyalar bulut servisinde saklanıyor.'");
