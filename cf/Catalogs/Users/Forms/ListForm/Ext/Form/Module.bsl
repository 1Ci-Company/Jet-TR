	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			...
			If StoredParameters.AdvancedPick Then
				...
				Items.EndAndClose.Title = NStr("en = 'Complete and close';tr = 'Tamamla ve kapat'");
			EndIf;
...
#If MobileClient Then
	If StoredParameters.UseGroups Then
		Items.GroupsGroup.Title = ?(Items.UserGroups.CurrentData = Undefined,
			NStr("en = 'User groups';tr = 'Kullanıcı grupları'"),
...
Procedure UserGroupsDrag(Item, DragParameters, StandardProcessing, String, Field)
	...
	If UsersCount = 1 Then
		...
		Else
			If AddToGroup Then
				Template = NStr("en = 'Group ""%1"" is marked for deletion. Do you want to add user ""%2"" to the group?';tr = 'Grup ""%1"" silinmek üzere işaretlenmiştir. Kullanıcıyı Etkinleştir""%2""bu gruba?'");
			ElsIf Move Then
	...
	Else
		...
		ElsIf Not GroupMarkedForDeletion Then
			If AddToGroup Then
				Template = NStr("en = 'Do you want to add %1 users to group ""%2""?';tr = '""%2"" grubuna users(%1) dahil edilsin mi?'");
			ElsIf Move Then
...
Procedure LockInvalidUsersInCollaborationSystem(Command)
	...
	If Not IsBlankString(ProcessingResult) Then
		MessageTemplate = NStr("en = 'Cannot disable users due to:
			|
			|%1';tr = 'Kullanıcılar şu nedenle devre dışı bırakılamıyor:
			|
			|%1'");
		ShowMessageBox(,
			StringFunctionsClientServer.SubstituteParametersToString(MessageTemplate, ProcessingResult),,
			NStr("en = 'Cannot disable users';tr = 'Kullanıcılar devre dışı bırakılamıyor'"));
	Else
		Status(NStr("en = 'Inactive users are
			|disabled.';tr = 'Aktif olmayan kullanıcılar
			|devre dışı bırakıldı.'"),,,PictureLib.Success32);
	EndIf;
EndProcedure
...
Function BlockInvalidUsersInTheInteractionSystemOnTheServer()
	...
	For Each InvalidUser In InvalidUsers Do
		...
		If BlockingResult <> Undefined Then
			...
			WriteLogEvent(NStr("en = 'Conversations.Disable inactive users';tr = 'Sohbetler.Pasif kullanıcıları devre dışı bırak'", Common.DefaultLanguageCode()),
				EventLogLevel.Error,,,
...
Procedure ChangeExtendedPickFormParameters()
	...
	ElsIf GetFunctionalOption("UseUserGroups") Then
		...
		Items.UsersList.Title                    = NStr("en = 'Users in group';tr = 'Gruptaki kullanıcılar'");
...
Procedure ConfigureUserGroupsUsageForm(GroupUsageChanged = False, 
	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			...
			If UsersGroupsSelection Then
				Title = ?(StoredParameters.AdvancedPick, StoredParameters.PickFormHeader,
					NStr("en = 'Pick users and groups';tr = 'Kullanıcıları ve grupları seç'"));
				CommonClientServer.SetFormItemProperty(Items,
					"SelectUser", "Title", NStr("en = 'Select users';tr = 'Kullanıcıları seç'"));
				CommonClientServer.SetFormItemProperty(Items,
					"SelectUsersGroup", "Title", NStr("en = 'Select groups';tr = 'Grupları seçin'"));
			Else
				Title = ?(StoredParameters.AdvancedPick, StoredParameters.PickFormHeader,
					NStr("en = 'Pick users';tr = 'Kullanıcıları seç'"));
		...
		Else
			// Selection mode.
			If UsersGroupsSelection Then
				Title = NStr("en = 'Select user or group';tr = 'Kullanıcı veya grubu seçin'");
				
				CommonClientServer.SetFormItemProperty(Items,
					"SelectUser", "Title", NStr("en = 'Select user';tr = 'Kullanıcı seçin'"));
			Else
				Title = NStr("en = 'Select user';tr = 'Kullanıcı seçin'");
			EndIf;
		EndIf;
	EndIf;
