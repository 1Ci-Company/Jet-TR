	...
	Except
		...
		Comment = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = 'When getting extension version parameter
			           ...
			           |%2';tr = 'Uzantı sürümü parametresi
			           |%1
			           |alınırken, depolama yerinden değer alınamadı:
			           |%2'"),
			ParameterName,
			ErrorProcessing.DetailErrorDescription(ErrorInfo));
		EventName = NStr("en = 'Configuration extensions.Get extension parameter';tr = 'Konfigürasyon uzantıları.Uzantı parametresini al'",
		...
		If ConfigurationChanged Or ExtensionsChanged Then
			Comment = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("en = 'When getting extension version parameter
				           ...
				           |%4';tr = 'Mevcut oturumda (eski metaveri sürümü %2 ile) değiştirmek üzere uzantı sürümü parametresi
				           |%1
				           |alınırken
				           |%3
				           |sistem, parametrenin
				           |%4 oturumunda değiştirildiğini tespit etti'"),
				ParameterName,
	 ...
	 Or Parameters.ConfigurationVersion <> Metadata.Version Then
		ErrorText =
			NStr("en = 'Cannot update all of the extension parameters
			           |because the configuration name or version was changed. Please restart the session.';tr = 'Adı veya yapılandırma sürümü değiştiğinden 
	 ...
	 Or Parameters.InstalledExtensions.Corrections <> InstalledExtensions.Corrections Then
		ErrorText =
			NStr("en = 'Cannot update all extension parameters
			           ...
			           |Retry the operation.';tr = 'Uzantıların tüm çalışma parametreleri güncellenemedi, 
			           |çünkü güncelleme görevi başlatılana kadar uzantıların içeriği tekrar değişti.
...
Procedure UpdateExtensionParameters(ExtensionsToCheck = Undefined, UnattachedExtensions = "", AsynchronousCallText = "") Export
	...
	JobDescription =
		NStr("en = 'The common form of the ""Fill in extension parameters"" extension';tr = 'Uzantının genel formu: Uzantıların çalışma parametrelerinin oldurulması'",
			Common.DefaultLanguageCode())
	...
	If TypeOf(Result) <> Type("Structure") Then
		ErrorText = NStr("en = 'The background job that prepares extensions did not return a result.';tr = 'Uzantıları hazırlayan arka plan işi sonuç vermedi.'");
		Raise ErrorText;
	EndIf;
...
Procedure FillinAllJobParametersLatestVersionExtensions() Export
	...
	Except
		...
		StartFillingWorkParametersExtensions(
			NStr("en = 'Restart due to an unsuccessful attempt to lock the parameter';tr = 'Başarısız parametre kilitleme denemesi nedeniyle yeniden başlatma'"));
		Return;
	EndTry;
	 ...
	 Or Catalogs.ExtensionsVersions.ExtensionsChangedDynamically() Then
		...
		StartFillingWorkParametersExtensions(
			NStr("en = 'Restart due to changing metadata of extensions or
			           |the configuration after startup before the update begins';tr = 'Başlatmadan sonra, günceleme başlamadan önce uzantıların veya
			           |konfigürasyonun metaverilerinin değişmesi nedeniyle yeniden başlatma'"));
		Return;
	EndIf;
			 ...
			 Or Not StandardSubsystemsServer.ThisErrorRequirementRestartSession(ErrorInfo) Then
				...
				If PreviousError = CurrentError Then
					ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("en = 'When attempting to fill in %1 from %2, the error of the previous attempt occurred again.';tr = '%1 / %2 doldurulmaya çalışılırken önceki denemedeki hata tekrarlandı.'",
							Common.DefaultLanguageCode()),
				...
				Else
					ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
						NStr("en = 'When attempting to fill in %1 from %2, the following error occurred:
						           |%3';tr = '%1 / %2 doldurulmaya çalışılırken hata oluştu:
						           |%3'",
							Common.DefaultLanguageCode()),
						Format(AttemptNumber, "NG="),
						Format(AttemptsNumber, "NG="),
			...
			If SessionRestartRequired Then
				...
				StartFillingWorkParametersExtensions(
					NStr("en = 'Restart due to changing metadata or
					           |extensions when filling the parameters';tr = 'Parametreler doldurulurken metaveri veya
					           |uzantı değişikliği nedeniyle yeniden başlatma'"));
				Return;
			EndIf;
	...
	If Not ValueIsFilled(PreviousError) Then
		Comment = NStr("en = 'Filled in successfully.';tr = 'Doldurulma başarıyla tamamlandı.'",
			Common.DefaultLanguageCode());
	...
	If Restart Then
		...
		StartFillingWorkParametersExtensions(
			NStr("en = 'Restart due to changing metadata or
			           |extensions after filling the parameters';tr = 'Parametreler doldurulduktan sonra metaveri veya
			           |uzantı değişikliği nedeniyle yeniden başlatma'"));
	EndIf;
...
Function ParameterFillingEventName() Export
	...
	Return NStr("en = 'Configuration extensions.Fill in extension parameters';tr = 'Konfigürasyon uzantıları. Uzantı çalışmasının parametrelerinin doldurulması'",
		Common.DefaultLanguageCode());
...
Function TaskNameFillingParameters() Export
	...
	Return NStr("en = 'Fill in extension parameters';tr = 'Çalışma uzantıların parametrelerinin doldurulması'",
		Common.DefaultLanguageCode());
...
Procedure ExecuteUpdateSplitDataInBackground(Parameters, FormIdentifier) Export
	...
	OperationParametersList.BackgroundJobDescription = NStr("en = 'Update separated service data';tr = 'Ayırılmış yardımcı verilerin güncellenmesi'");
	   ...
	   And Not Common.SeparatedDataUsageAvailable() Then
		ErrorText =
			NStr("en = 'Cannot update extension parameters. Reason:
			           |Cannot update in shared mode.';tr = 'Uzantı parametreleri güncellenemiyor. Nedeni:
			           |Ortak modda güncelleme yapılamıyor.'");
		Raise ErrorText;
	EndIf;
...
Procedure EnableFillingExtensionsWorkParameters(Run = True, EnableDefinitely = False) Export
	...
	If Run Then
		StartFillingWorkParametersExtensions(
			NStr("en = 'Startup with app parameter setup enabled';tr = 'Etkinleştirilmiş uygulama parametre ayarlarıyla başlatma'"));
	EndIf;
...
Procedure AddAdditionalDetails(Comment)
	...
	AdditionalInfo = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = '*** Additional information records ***
		           ...
		           |%5';tr = '*** Ek bilgiler ***
		           |
		           |1. Bağlanan uzantılar (düzeltmeler dahil):
		           |%1
		           |
		           |2. Devre dışı uzantılar (düzeltmeler dahil):
		           |%2
		           |
		           |3. Veritabanının tüm uzantıları (düzeltmeler dahil):
		           |%3
		           |
		           |4. Uzantıların versiyonu ""%4""
		           |%5'",
			Common.DefaultLanguageCode()),
