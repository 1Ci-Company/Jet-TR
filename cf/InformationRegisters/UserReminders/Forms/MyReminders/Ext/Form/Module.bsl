...
Procedure ListSelection(Item, RowSelected, Field, StandardProcessing)
	If Field.Name = "Source" Then
		...
		Else
			ShowMessageBox(, NStr("en = 'Please specify the reminder source.';tr = 'Hatırlatıcı kaynağı belirlenmemiş.'"));
		EndIf;
	EndIf;
EndProcedure
...
Procedure DeleteReminder()
	...
	DialogButtons.Add(DialogReturnCode.Yes, NStr("en = 'Delete';tr = 'Sil'"));
	DialogButtons.Add(DialogReturnCode.Cancel, NStr("en = 'Do not delete';tr = 'Silme'"));
	NotifyDescription = New NotifyDescription("DeleteReminderCompletion", ThisObject);
	
	ShowQueryBox(NotifyDescription, NStr("en = 'Delete the reminder?';tr = 'Hatırlatıcı silinsin mi?'"), DialogButtons);
