	...
	If SettingsToClearRadioButton = "SelectedSettings2" Then
		If UsersToClearSettingsRadioButtons <> "SelectedUsers1"
		 Or UsersCount <> 1 Then
			...
			ShowMessageBox(,NStr("en = 'To clear individual settings, select one user.';tr = 'Kişisel ayarları temizlemek için tek bir kullanıcı seçin.'"));
		Else
...
Procedure UpdateUsersSelectionRef(Form)
	...
	If Form.UsersCount = 0 Then
		Form.Items.SelectSettings.Title = NStr("en = 'Select';tr = 'Seç'");
...
Function SettingsClearing()
	...
	If UsersToClearSettingsRadioButtons = "SelectedUsers1"
		...
		CommonClient.MessageToUser(
			NStr("en = 'Select the users whose
				|settings you want to clear.';tr = 'Ayarlarını temizlemek istediğiniz
				|kullanıcıları seçin.'"), , "Source");
		Return False;
	EndIf;
	...
	If UsersToClearSettingsRadioButtons = "SelectedUsers1" Then
		...
		If UsersCount = 1 Then
			SettingsClearedForNote = NStr("en = 'user ""%1""';tr = 'kullanıcı ""%1""'");
		...
		Else
			SettingsClearedForNote = NStr("en = '%1 users';tr = '%1 kullanıcıları'");
			SettingsClearedForNote = StringFunctionsClientServer.SubstituteParametersToString(SettingsClearedForNote, UsersCount);
		EndIf;
		
	Else
		SettingsClearedForNote = NStr("en = 'all users';tr = 'tüm kullanıcılar'");
	EndIf;
	
	If SettingsToClearRadioButton = "SelectedSettings2"
		And SettingsCount = 0 Then
		CommonClient.MessageToUser(
			NStr("en = 'Select the settings that you want to clear.';tr = 'Temizlenecek ayarları seçin.'"), , "SettingsToClearRadioButton");
		Return False;
	EndIf;
	...
	If SettingsToClearRadioButton = "SelectedSettings2" Then
		...
		If SettingsCount = 1 Then
			...
			ExplanationText = NStr("en = '""%1"" cleared for %2';tr = '""%1"", %2 için temizlendi'");
	...
	ElsIf SettingsToClearRadioButton = "ClearAll" Then
		...
		ExplanationText = NStr("en = 'All settings are cleared for %1';tr = 'Tüm ayarlar %1 temizlendi'");
	...
	ElsIf SettingsToClearRadioButton = "ObsoleteSettings" Then
		...
		ExplanationText = NStr("en = 'Obsolete settings %1 are cleared';tr = 'Eski %1 ayarları temizlendi'");
		ExplanationText = StringFunctionsClientServer.SubstituteParametersToString(ExplanationText, SettingsClearedForNote);
		ShowUserNotification(NStr("en = 'Clear settings';tr = 'Temizleme ayarları'"), , ExplanationText, PictureLib.DialogInformation);
	EndIf;
