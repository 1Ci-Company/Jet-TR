...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ReportsClientServer.DisplayReportState(
		ThisObject, NStr("en = 'To run the report, click ""Generate"".';tr = 'Raporu çalıştırmak için ""Oluştur""a tıklayın.'"));
...
#If WebClient Then
	If ValueIsFilled(PathToExternalReportFileAtClient) Then
		ErrorText = NStr("en = 'For this action, start the client application';tr = 'Bu eylem için istemci uygulamasını başlatın'");
		Raise ErrorText;
	EndIf;
#EndIf
...
Procedure OnLoadVariantAtServer(NewDCSettings)
	...
	If IsMobileClient Then
		...
		If ResourcePlacement.Use And ResourcePlacement.Value = DataCompositionResourcesPlacement.Vertically Then
			Items.DecorationEditResourcePlacement.Title = NStr("en = 'Horizontal';tr = 'Yatay'");
		EndIf;
	EndIf;
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	For Each SettingItem In SettingsItems Do
		...
		If StartDate.AutoMarkIncomplete = True And Not ValueIsFilled(Value.StartDate)
			...
			ErrorText = NStr("en = 'The period is not specified.';tr = 'Dönem belirtilmedi.'");
			DataPath = StartDate.DataPath;
		ElsIf Value.StartDate > Value.EndDate Then
			ErrorText = NStr("en = 'Period end must be later than period start.';tr = 'Dönem sonu, başlangıcından sonra olmalıdır'");
...
Procedure FactorExtendedTooltipURLProcessing(Item, FormattedStringURL,
	...
	Else
		ShowMessageBox(, NStr("en = 'Advanced filter settings are not available';tr = 'Filtrelerin genişletilmiş ayar olanakları kullanılamaz'"));
	EndIf;
...
Procedure SendByEmail(Command)
	...
	If StatePresentation.Visible = True 
		...
		QueryText = NStr("en = 'Report not generated. Do you want to generate the report?';tr = 'Rapor oluşturulmadı. Oluşturulsun mu?'");
...
Procedure ImportSchema(Command)
	...
	ImportParameters.Dialog.Filter = NStr("en = 'XML files (*.xml) |*.xml';tr = 'XML (*.xml) |*.xml dosyalar'");
...
#Else
	ShowMessageBox(, NStr("en = 'To edit composition schema, run the application in thick client mode.';tr = 'Düzen şemasını düzenlemek için uygulamayı kalın istemci modunda çalıştırın.'"));
#EndIf
EndProcedure
...
Procedure SaveReport(Command)
	...
	Dialog.Title = NStr("en = 'Save report result';tr = 'Rapor sonucunun kaydedilmesi'");
...
Procedure SaveReportSnapshot(Command)
	...
	If ReportSettings.ResultProperties.SettingsComposer = Undefined Then
		WarningText = NStr("en = 'Generate the report before saving the snapshot.';tr = 'Anlık görüntüyü kaydetmeden önce raporu oluşturun.'");
...
Procedure Attachable_ImportReportOption(Command)
	...
	If FoundItems.Count() = 0 Then
		ShowMessageBox(, NStr("en = 'The report option does not exist.';tr = 'Rapor seçeneği mevcut değil.'"));
		Return;
	EndIf;
...
Procedure AfterGenerate(Result, GenerationParameters) Export
	...
	If Result = Undefined Then
		...
		ShowGenerationErrors(NStr("en = 'Report generation is canceled by the administrator';tr = 'Raporun oluşturulması yönetici tarafından iptal edildi'"));
		ShowUserNotification(NStr("en = 'Report is not generated';tr = 'Rapor düzenlenmedi'"),
	...
	ElsIf Result.Status = "Completed2" Then
		...
		ShowUserNotification(NStr("en = 'Report is generated';tr = 'Rapor düzenlendi'"),
			?(Window <> Undefined, Window.GetURL(), Undefined), Title);
...
Function ReportGenerationResult(Val GeneratingOnOpen, Directly, AfterWarning)
	...
	ExecutionParameters.BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = 'Running report: %1';tr = 'Raporun yürütülmesi: %1'"), ReportName);
	...
	Else
		ReportsClientServer.DisplayReportState(
			ThisObject, NStr("en = 'Generating report…';tr = 'Rapor oluşturuluyor...'"), PictureLib.TimeConsumingOperation48);
	EndIf;
	Return Result;
...
Procedure ImportReportGenerationResult()
	If Not IsTempStorageURL(BackgroundJobStorageAddress) Then
		ShowGenerationErrors(NStr("en = 'Cannot generate the report.';tr = 'Rapor oluşturulamadı'"));
		Return;
	EndIf;
	...
	If Result = Undefined Then
		ShowGenerationErrors(NStr("en = 'Cannot generate the report (empty result)';tr = 'Rapor düzenlenemedi (boş sonuç)'"));
		Return;
	EndIf;
...
Procedure ApplyPassedSettings(SettingsDescription)
	If VariantModified Then
		ShowMessageBox(, NStr("en = 'The report option was changed.
			|Please save the changes before applying the settings.';tr = 'Rapor seçeneği değiştirildi.
			|Ayarlar uygulanmadan önce değişiklikleri kaydedin.'"));
		Return;
	EndIf;
...
Function SuitableUniversalSearchValues(Query)
	...
	If SuitableValues.Count() = 0 Then
		...
		If StrLen(SearchParameters.SearchString) > 2 Then
			Message = NStr("en = 'No matches found';tr = 'Eşleşmeler bulunamadı'");
		Else
			Message = NStr("en = 'Continue typing…';tr = 'Girişe devam edin...'");
		EndIf;
...
Procedure FindSuitableValuesBooleanUniversalSearch(SuitableValues, SearchFieldsByType, SearchParameters)
	...
	If StrCompare(SearchParameters.SearchString, NStr("en = 'Yes';tr = 'Evet'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'True';tr = 'True'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'Enabled';tr = 'Etkin'")) = 0 Then
		For Each FieldForSearch In SearchFields Do
			SearchProperties = UniversalSearchProperties(FieldForSearch.Key, True, SearchParameters.ControlCharacters);
			SearchOptions = New Array;
			SearchOptions.Add(SearchProperties);
			ValuesSuitableForBoolean.Add(SearchOptions, ViewSearchResultClient(
				FieldForSearch.Value, SearchProperties.ComparisonType, SearchProperties.RightValue));
		EndDo;
	ElsIf StrCompare(SearchParameters.SearchString, NStr("en = 'No';tr = 'Hayır'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'False';tr = 'False'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'Disabled';tr = 'Devre dışı'")) = 0 Then
		For Each FieldForSearch In SearchFields Do
			SearchProperties = UniversalSearchProperties(FieldForSearch.Key, False, SearchParameters.ControlCharacters);
			SearchOptions = New Array;
			SearchOptions.Add(SearchProperties);
...
Procedure FindSuitableValuesStringUniversalSearch(SuitableValues, SearchFieldsByType, SearchParameters)
	...
	If StrCompare(SearchParameters.SearchString, NStr("en = 'Yes';tr = 'Evet'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'True';tr = 'True'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'Enabled';tr = 'Etkin'")) = 0
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'No';tr = 'Hayır'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'False';tr = 'False'")) = 0 
		Or StrCompare(SearchParameters.SearchString, NStr("en = 'Disabled';tr = 'Devre dışı'")) = 0 Then
		Return;
	EndIf;
...
Function RepresentationOfTheUniversalLinkSearch(Data, FieldForSearch, Condition, SearchString)
	...
	If ValueIsFilled(Data.AdditionalData) Then
		ValuePresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = '%1 (%2)';tr = '%1 (%2)'"), String(Data.Ref), Data.AdditionalData);
	EndIf;
...
Function ConditionFragmentTemplate(Condition)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = '%1<span style=""color: %2"">%3</span>%4';tr = '%1<span style=""color: %2"">%3</span>%4'"), " ", "HiddenReportOptionColor", 
...
Function SearchStringFragmentTemplate(SearchString)
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = '<span style=""color: %1; font: %2"">%3</span>';tr = '<span style=""color: %1; font: %2"">%3</span>'"), "MyReportsOptionsColor", "ImportantLabelFont",
		SearchString);
...
Function PropertiesOfTheMenuOfLevelsOfGroupingsOfAComplexReport(IndexOfTheReportStructure)
	...
	For GroupingLevel = 1 To NumberOfGroupingLevels Do
		...
		Properties.RepresentationOfTheGroupingLevel = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = '%1 level';tr = '%1 seviyesi'"), GroupingLevel);
...
Function ReportGroupingsBasedOnAFieldWithAHierarchicalGrouping(IndexOfTheReportStructure, FieldsWithHierarchicalGrouping,
	...
	GroupingViewTemplate = NStr("en = '%1 - level %2';tr = '%1 - seviye %2'");
...
Procedure SaveReportAfterFilenameSelected(Result, Context) Export
	...
	If Not ValueIsFilled(FullNameOfTheReportFile) Then
		...
		FormatsList.ShowChooseItem(OnCloseNotifyDescription, NStr("en = 'Select a save format';tr = 'Kaydetme formatını seçin'"),
			DefaultFormat);
	Else
...
Procedure SaveReportAfterSavedReportResults(Result, FullNameOfTheReportFile) Export
	...
	ShowUserNotification(NStr("en = 'The report is saved to the file';tr = 'Rapor dosyaya kaydedildi'"), Handler, FullNameOfTheReportFile);
...
Procedure SetVisibilityAvailability()
	...
	If DetailsMode Then
		Title = Title + " (" + NStr("en = 'Details';tr = 'Ayrıntılar'") + ")";
	EndIf;
EndProcedure
...
Procedure LoadVariant(VariantKey, ClearStackSettings = True)
	...
	ReportsClientServer.DisplayReportState(	ThisObject, 
		NStr("en = 'Another report option is selected. To run the report, click ""Generate"".';tr = 'Başka bir rapor seçeneği seçildi. Raporu almak için ""Oluştur""a tıklayın.'"),
...
Procedure DefineBehaviorInMobileClient()
	...
	Items.CommandsAndIndicators.Title = NStr("en = 'indicators';tr = 'göstergeler'");
...
Function EventsSettings()
	...
	EventsSettings.Insert("FilterAndGenerate", NStr("en = 'Filter…';tr = 'Filtrele...'"));
	EventsSettings.Insert(ReportsOptionsInternalClientServer.NameEventFormSettings(), 
		NStr("en = 'Advanced setting';tr = 'Gelişmiş ayar'"));
	EventsSettings.Insert(ReportsOptionsInternalClientServer.EventNameQuickSettingsChangesContent(), 
		NStr("en = 'Quick settings change';tr = 'Hızlı ayarların içerik değişimi'"));
...
Function RefinementsEventSettings()
	...
	Prefixes = New Map;
	Prefixes.Insert("HighlightInYellow", NStr("en = 'Highlight in yellow';tr = 'Sarı ile vurgula'"));
	Prefixes.Insert("HighlightInRed", NStr("en = 'Highlight in red';tr = 'Kırmızı ile vurgula'"));
	Prefixes.Insert("HighlightInGreen", NStr("en = 'Highlight in green';tr = 'Yeşil ile vurgula'"));
	Prefixes.Insert("ApplyAppearanceMore", NStr("en = 'Format (more)';tr = 'Format (diğer)'"));
...
Procedure ShowGenerationErrors(ErrorInfo)
	If TypeOf(ErrorInfo) = Type("ErrorInfo") Then
		...
		DetailErrorDescription = NStr("en = 'The report is not generated due to:';tr = 'Rapor şu nedenle oluşturulamadı:'") + Chars.LF
...
Procedure SaveAsNewOrOverwriteExistingReportOptionAndContinue(Command)
	...
	If Not Items.SaveOptionAs.Visible Then
		ShowMessageBox(, NStr("en = 'Cannot perform the action as saving report options is unavailable.';tr = 'Rapor seçenekleri kaydedilemediği için işlem gerçekleştirilemiyor.'"));
		Return;
	EndIf;

	If Items.SaveVariant.Visible Then
		Buttons = New ValueList;
		Buttons.Add("SaveAndResume", NStr("en = 'Save and continue';tr = 'Kaydet ve devam et'"));
		Buttons.Add("Cancel", NStr("en = 'Cancel';tr = 'İptal'"));
		QuestionParameters = StandardSubsystemsClient.QuestionToUserParameters();
		...
		StandardSubsystemsClient.ShowQuestionToUser(
			...
			NStr("en = 'Do you want to save the report option?';tr = 'Rapor seçeneği kaydedilsin mi?'"), Buttons, QuestionParameters);
		Return;
	Else
		FormParameters = New Structure;
...
Procedure SaveReportOption(AfterAddedNew = False)
	...
	ShowUserNotification(NStr("en = 'The report option is saved';tr = 'Rapor seçeneği kaydedildi'"), 
		?(Window <> Undefined, Window.GetURL(), Undefined), Title);
