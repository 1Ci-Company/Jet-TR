...
Function VersionCacheData(Val Id, Val DataType, Val ReceivingParameters, Val UseObsoleteData = True) Export
	...
	If UpdateRequired2 Then
		...
		Else
			...
			JobDescription = StringFunctionsClientServer.SubstituteParametersToString(NStr("en = 'Version cache update. Entry ID: %1. Data type: %2.';tr = 'Sürüm önbelleği güncelleniyor. Kayıt kimliği: %1. Veri tipi: %2.'"),
				Id,
				DataType);
		...
		If RereadDataRequired Then
			...
			If Result.IsEmpty() Then
				MessageTemplate = NStr("en = 'Version cache update error. The data is not received.
					|Entry ID: %1
					|Data type: %2';tr = 'Sürümler önbelleği güncellenirken bir hata oluştu. Veri alınmıyor.
					|Kayıt kimliği: %1
					|Veri türü: %2'");
...
Function PrepareVersionCacheData(Val DataType, Val ReceivingParameters) Export
	...
	Else
		TextTemplate1 = NStr("en = 'Unknown version cache data type: %1.';tr = 'Sürüm önbelleklerinin bilinmeyen veri türü: %1'");
...
Function WSDefinitions(Val WSDLAddress, Val UserName, Val Password, Val Timeout = 10, Val SecureConnection = Undefined)
		...
		Except
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("en = 'Failed to get WS definitions at
				           |%1.
				           |Reason:
				           |%2';tr = 'Aşağıdaki nedenle 
				           |%1
				           | adresinde WS-tanımlamalar elde edilemedi: 
				           |%2'"),
				WSDLAddress,
	...
	Except
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = 'Failed to get WS definitions from cache.
			           |Reason:
			           |%1';tr = 'WS-tanımlamaları önbellekten alınamadı.
			           |Nedeni: 
			           |%1'"),
			ErrorProcessing.BriefErrorDescription(ErrorInfo()));
		Raise ErrorText;
	...
	Except
		WriteLogEvent(NStr("en = 'Getting WSDL';tr = 'WSDL alın'", Common.DefaultLanguageCode()),
...
Function GetInterfaceVersionsToCache(Val ConnectionParameters, Val InterfaceName)
	...
	If Not ConnectionParameters.Property("URL") 
		...
		Raise(NStr("en = 'The service URL is not set.';tr = 'Hizmetin URL''si belirlenmemiş.'"));
	EndIf;
...
Function GetWSDL(Val Address, Val UserName, Val Password, Val Timeout, Val SecureConnection = Undefined)
	...
	Else
		Raise(NStr("en = 'The ""Network download"" subsystem is unavailable.';tr = '""İnternet''ten dosya alma"" alt sistemi kullanılamaz.'"),
	...
	If Not FileDetails.Status Then
		Raise(StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = 'Cannot get web service details file %1 due to:
				|%2';tr = 'Bu nedenle %1 ağ servisinin açıklama dosyası alınamadı:
				|%2'"),
	...
	Except
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = 'Cannot get web service details file %1 due to:
				...
				|
				|Diagnostics result:
			    |%3';tr = 'Bu nedenle %1 ağ servisinin açıklama dosyası alınamadı:
				|%2
				|
				|Tanımlama sonucu:
				|%3'"),
		...
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = '%1
			           ...
			           |Zaman aşımı: %3'"),
			ErrorText,
			Format(SecureConnection, NStr("en = 'BF=No; BT=Yes';tr = 'BF=Hayır; BT=Evet'")),
			Format(Timeout, "NG=0"));
	...
	If Definitions.Services.Count() = 0 Then
		Raise(StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = 'Cannot get the web service description file.
			           ...
			           |%1';tr = 'Bu nedenle ağ servisinin açıklama dosyası alınamadı:
			           |Alınan dosyada hiçbir servis açıklaması yok.
			           |Muhtemelen açıklama dosyasının adresi yanlış belirtilmiş:
			           |%1'"),
			Address),
