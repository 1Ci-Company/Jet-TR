...
Procedure MetadataObjectsTreeBeforeDeleteRow(Item, Cancel)
	...
	If SettingToDelete <> Undefined Then
		...
		If SettingToDeleteParent <> Undefined And SettingToDeleteParent.DetailedInfoAvailable Then
			...
			QueryText = NStr("en = 'If you delete the setting, you will not be able
				|to clean up files according to the rules defined in it. Continue?';tr = 'Ayarların silinmesi, belirlenmiş kurallara göre
				|dosyaların silinmesinin durdurulmasına yol açar. Devam edilsin mi?'");
			NotifyDescription = New NotifyDescription("DeleteSettingItemCompletion", ThisObject);
			ShowQueryBox(NotifyDescription, QueryText, QuestionDialogMode.YesNo, , DialogReturnCode.No, NStr("en = 'Warning';tr = 'Uyarı'"));
			Return;
	...
	MessageText = NStr("en = 'Advanced file cleanup settings are unavailable for this object.';tr = 'Bu nesne için genişletilmiş dosya temizleme ayarı öngörülmemiştir.'");
...
Function CanClearVersions()
	If Items.MetadataObjectsTree.SelectedRows.Count() = 1 Then
		...
		If Not CurrentData.IsFile
			...
			ShowMessageBox(, NStr("en = 'File versions are not stored for this object.';tr = 'Bu nesne için dosya sürümleri kaydedilmemektedir.'"));
			Return False;
		EndIf;
	EndIf;
...
Procedure Clear(Command)
	...
	ShowQueryBox(Notification, NStr("en = 'Clean up unused files?
		...
		|You might want to create a backup of the infobase and network file storage volumes before deleting.';tr = 'Kullanılmayan dosyalar silinsin mi?
		|
		|Kullanılmayan dosyalar yaptığınız ayarlara bağlı olarak tamamen silinecek.
		|Silmeden önce infobase''i ve ağ dosya depolama birimlerini yedeklemek isteyebilirsiniz.'"), 
...
Procedure CalculateDeletableFilesInfo()
	...
	ExecutionParameters.RefinementErrors =
		NStr("en = 'Cannot calculate the total size of files to delete:';tr = 'Silinecek toplam dosya boyutu hesaplanamıyor:'");
...
Procedure DisplayTotals(TheAmountOfFilesBeingDeleted, IrrelevantFilesVolume)
	Items.DeletedFilesVolume.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = 'Files being deleted: %1 MB';tr = 'Silinecek dosyalar: %1 Mb'"),
	...
	Items.IrrelevantFilesVolume.Title = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("en = 'Unused files: %1 MB';tr = 'Gereksiz dosyalar: %1 Mb'"),
	...
	For Each Type In TypesTable Do
		...
		If AllCatalogs.ContainsType(Type.FileOwner) Then
			If CatalogsNode = Undefined Then
				...
				CatalogsNode.ObjectDescriptionSynonym = NStr("en = 'Catalogs';tr = 'Kataloglar'");
			EndIf;
		...
		ElsIf AllDocuments.ContainsType(Type.FileOwner) Then
			If DocumentsNode = Undefined Then
				...
				DocumentsNode.ObjectDescriptionSynonym = NStr("en = 'Documents';tr = 'Belgeler'");
			EndIf;
		...
		ElsIf BusinessProcesses.AllRefsType().ContainsType(Type.FileOwner) Then
			If BusinessProcessesNode = Undefined Then
				...
				BusinessProcessesNode.ObjectDescriptionSynonym = NStr("en = 'Business processes';tr = 'İş süreçleri'");
			EndIf;
...
Procedure RunScheduledJob()
	...
	Else
		...
		JobParameters.BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("en = 'Manual start: %1';tr = 'Manuel başlatma: %1'"), ScheduledJobMetadata1.Synonym);
...
Procedure AddSettingsByOwner(ValueSelected)
	...
	For Each Setting In ValueSelected Do
		...
		DetalizedSetting.FilterRule = NStr("en = 'Change rule';tr = 'Kural değiştir'");
	EndDo;
...
Procedure ConfigureFilePurgeModes()
	...
	Items.AutomaticallyCleanUpUnusedFiles.Title = ?(UseVolumes, 
		NStr("en = 'Clean up automatically:';tr = 'Otomatik olarak temizle:'"), NStr("en = 'Clean up unused files automatically:';tr = 'Kullanılmayan dosyaları otomatik temizle:'"));
